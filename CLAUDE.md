...
- **`cpuSlowdownMultiplier: 4`** — fixed CPU throttling multiplier instead of Lighthouse's automatic benchmark-based calibration. GitHub Actions runners are shared machines; available CPU speed varies between jobs, so the auto-calibration produces a different throttling level each run — variability that comes from the runner, not the site. Value 4 is Lighthouse's own default for mobile, so the simulation level is unchanged but now consistent across all runs.
